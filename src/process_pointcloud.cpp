...
namespace pointcloud_obstacle_detection{
  ...
  * */
  ...
  CloudXYZ ProcessPointCloud::FilterCloud(CloudXYZ cloud, bool downSampleInputCloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
  {

    // Time segmentation process
    auto startTime = std::chrono::steady_clock::now();
    CloudXYZ cloud_filtered(new pcl::PointCloud<pcl::PointXYZ>());
  ...
  CloudPair ProcessPointCloud::SeparateClouds(pcl::PointIndices::Ptr inliers, CloudXYZ cloud)
  {
    // TODO: Create two new point clouds, one cloud with obstacles and other with segmented plane
     CloudXYZ obstCloud(new pcl::PointCloud<pcl::PointXYZ>());
     CloudXYZ planeCloud(new pcl::PointCloud<pcl::PointXYZ>());
    ...
    extract.filter (*obstCloud);
      ...
      CloudPair segResult(obstCloud, planeCloud);
      return segResult;
  }


  
  CloudPair ProcessPointCloud::SegmentPlane(CloudXYZ cloud, int maxIterations, float distanceThreshold)
  {
      // Time segmentation process
      auto startTime = std::chrono::steady_clock::now();
    ...
    std::pair< CloudXYZ,  CloudXYZ> segResult = SeparateClouds(inliers,cloud);
      auto endTime = std::chrono::steady_clock::now();
      auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
      return segResult;
  }
  ...
  * */
  ...
  CloudPair ProcessPointCloud::SegmentPlane_RANSAC(CloudXYZ cloud, int maxIterations, float distanceThreshold)
  {
      // Time segmentation process
      auto startTime = std::chrono::steady_clock::now();
    ...
    /*Buffers to hold cloud and object points*/
     CloudXYZ cloudInliers(new pcl::PointCloud<pcl::PointXYZ>());
     CloudXYZ cloudOutliers(new pcl::PointCloud<pcl::PointXYZ>());
    ...
    * or else copy the point to cloudOutliers*/
    ...
    CloudPair segResult(cloudOutliers, cloudInliers);
  ...
  CloudVector ProcessPointCloud::Clustering(CloudXYZ cloud, float clusterTolerance, int minSize, int maxSize)
  {

    // Time clustering process
    auto startTime = std::chrono::steady_clock::now();

    CloudVector clusters;
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
      {
       CloudXYZ cloud_cluster (new pcl::PointCloud<pcl::PointXYZ>);
  ...
  * */
  ...
  void ProcessPointCloud::Proximity(CloudXYZ cloud,std::vector<int> &cluster,std::vector<bool> &processed_f,int idx, KdTree_euclidean* tree,float distanceTol, int maxSize)
  {
  ...
  * */
  ...
  std::vector<std::vector<int>> ProcessPointCloud::euclideanCluster(CloudXYZ cloud,  KdTree_euclidean* tree, float distanceTol, int minSize, int maxSize)
  {
  ...
  * */
  ...
  CloudVector ProcessPointCloud::Clustering_euclideanCluster(CloudXYZ cloud, float clusterTolerance, int minSize, int maxSize)
  {
  ...
  void ProcessPointCloud::savePcd( CloudXYZ cloud, std::string file)
  {
      pcl::io::savePCDFileASCII (file, *cloud);
  }
  
  CloudXYZ ProcessPointCloud::loadPcd(std::string file)
  {
